<script>
    import { gsap } from "gsap";
    import { onMount } from "svelte";

    let phrase = ['Hi! My Name is Edgar Rodriguez!', 'Im a Computer scientist'];
    let typedChars = [];
    let index = 0;
    let i = 0;
    let typing;

    const type = () => {
        if (typedChars.length < phrase[i].length){
            if (index < phrase[i].length) {
                typedChars += phrase[i][index];
                index++;
                setTimeout(type, 100);
            }
            else{
                clearInterval(typing);
            }
        }
        else{
            if (i+1 < phrase.length){
                setTimeout(clearTyping, 800);
                setTimeout(type, 800);
                index = 0;
                i++;
            }
            else{
                
                clearInterval(typing);
            }
        }

    };

    const clearTyping = () => {
        typedChars = '';
    };
    
    const startTyping = () => {
            typing = setInterval(type(), 800);
    };
    
    //  phrase.forEach(element => {
    //      while (typedChars.length < element.length) {
    //          startTyping(element);
    //      }
    //  });

    startTyping(phrase);
    let box;
    onMount(() => {
        gsap.from(box, {backgroundColor: "#000000", duration: 10});
    });
</script>

<div bind:this={box} class="background min-h-screen flex flex-col items-center justify-center overflow-hidden ">
    <div class=" flex flex-col h-full w-full backdrop-blur-[2px] ">
    </div>
        <div class="absolute text-8xl p-12 font-Dalton text-black tracking-widest font-medium">
            <span >{typedChars.toUpperCase()}</span>
        </div>
</div>

<style>
    .background {
        position:relative;
  width: 100vw;
  height: 100vh;
  overflow: hidden;
  background:repeating-linear-gradient(90deg, rgba(255, 255, 255, 0.984) -2px,rgba(0, 0, 14, 0.406) 0px, transparent, rgba(250, 250, 254, 0.406) 20px), radial-gradient(ellipse at center, #fff2bc 0%, #7d420f 100%);

}
.background::after {
  content:"";
  position: absolute;
  width:400vw;
  height: 400vh;
  background-image:url(data:image/png;base64,);
  animation:grain .9s steps(5) infinite;
}
@keyframes grain {
  0%, 100% {transform: translate(0,0);}
  10% {transform: translate(-5%,-10%);}
  30% {transform: translate(3%,-15%);}
  50% {transform: translate(12%,9%);}
  70% {transform: translate(9%,4%);}
  90% {transform: translate(-1%,7%);}
}
</style>
