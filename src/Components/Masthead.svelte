<script>
    ...
    let phrase = ['Hi! My Name is Edgar Rodriguez!', 'Im a Computer scientist'];
    let typedChars = "";
    let typedPhrases = "";
    let index = 0;
    let i = 0;
    ...
    const type = () => {
        if (typedChars.length < phrase[i].length){
            if (index < phrase[i].length) {
                typedChars += phrase[i][index];
                index++;
                setTimeout(type, 100);
            }
            else{
    ...
    const addPhrase = () => {
        typedPhrases = typedChars.toUpperCase();
    };
    ...
    const startTyping = () => {
            typing = setInterval(type(), 800);
    };
    
    //  phrase.forEach(element => {
    //      while (typedChars.length < element.length) {
    //          startTyping(element);
    //      }
    //  });
    ...
    let box;
    onMount(() => {
        gsap.from(box, {backgroundColor: "#000000", duration: 10});
    });
</script>

<div bind:this={box} class="backgroundMastHead min-h-screen flex flex-col items-center justify-center overflow-hidden ">
    <div class=" flex flex-col h-full w-full backdrop-blur-[2px] ">
    </div>
        <div class="absolute text-5xl md:text-8xl p-12 font-Dalton text-black tracking-widest font-medium">
            <span >{typedPhrases}</span>
            <br>
            <span >{typedChars.toUpperCase()}</span>
        </div>
</div>
...
<style>
  ...
  height: 100vh;
  overflow: hidden;
  background:repeating-linear-gradient(90deg, rgba(255, 255, 255, 0.984) -2px,rgba(0, 0, 14, 0.406) 0px, transparent, rgba(250, 250, 254, 0.406) 20px), radial-gradient(ellipse at center, #fff2bc 0%, #7d420f 100%);

}
.backgroundMastHead::after {
  content:"";
  position: absolute;
  width:200vw;
  height: 200vh;
